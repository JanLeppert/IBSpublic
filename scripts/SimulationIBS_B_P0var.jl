...
# for Plot 6 (resolution ratio over gT for different gas decompression P>1), higher acuracy necessary, longer calculation time
params = Dict(
                :gT => collect(-10:0.1:0),          # variation of gT
                :rT => 10.0.^(-0.4),                # constant rT
                :ϑ₀ => 1,
                :h => 1e-4,
                :ϑchar => [-1,0,1,2,4,6],           # variation of solutes
                :char => NaN,
                :C => 0,
                :Δϑchar => 1/300,
                :s₀ => 0,
                :guM => 0,
                :P => [1, 1.5, 2, 3, 5, 7, 10, 20, 50, 100, 200, 500, 1000, 2000, 1e4, 1e5, 1e6],
                ...
                :abstol => 1e-12,                   # higher acuracy necessary, default: 1e-8
                :reltol => 1e-9                     # higher acuracy necessary, default: 1e-5
)
...
for (i, d) in enumerate(dicts)
    f = makesim(d)
    @tagsave(datadir("simulation", "SimulationIBS_B_Pvar", savename(d, "bson", scientific=3, ignores=(:char, :C, :retention_model, :retention_difference, :char_model, :comparison))), f, safe=false)
end
