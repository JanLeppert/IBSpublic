...
# for Plot 3 (resolution ratio over gT and rT) and Plot 4 (performance ratios and chromatogramms over gT)
params = Dict(
                :gT => collect(-10:0.1:0),          # variation of gT
                :rT => 10.0.^(-3:0.1:1),            # variation of rT
                :ϑ₀ => 1,
                :h => 1e-4,
...
for (i, d) in enumerate(dicts)
    f = makesim(d)
    @tagsave(datadir("simulation", "SimulationIBS_B_gTvar_rTvar", savename(d, "bson", scientific=3, ignores=(:char, :C, :retention_model, :retention_difference, :char_model, :comparison))), f, safe=false)
end
