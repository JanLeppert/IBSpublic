...
df = collect_results!(datadir("simulation", "SimulationIBS_B"))
...
hold(false)
p2 = Figure()
plot(df_guM_var[1].gT, df_guM_var[1].RS2 ./ df_ibs.RS2)
hold(true)
xlabel("g_T")
ylabel("Xi(R_{S})")
for i=2:length(guM_var)
    plot!(p2, df_guM_var[i].gT, df_guM_var[i].RS2./df_ibs.RS2)
end
draw(p2)
legend("0.0", "1.0", "1.6", "1.8", "1.9", location="upper left")
# plot þR0 over gT
...
for i=1:length(P_var)
    df_P_var[i] = sort(filter([:guM, :P, :abstol] => (guM, P, abstol) -> guM==0 && P==P_var[i] && abstol==1e-10, dropmissing(df)), [:gT])
end
# filter the IBS solutions
df_ibs = filter([:guM, :gT, :s₀, :P, :abstol] => (guM, gT, s₀, P, abstol) -> guM==0 && gT==0 && s₀==0 && P==1 && abstol==1e-10, dropmissing(df))
#--------------------------------------------------------------------------------------------------
# Plots
# Resolution
hold(false)
...
draw(p4)
legend("1", "1.01", "1.5", "2.0", "3.0", "5.0", "10", "100", "1000", location="outer upper right")
# Resolution Ratio
hold(false)
p5 = Figure()
