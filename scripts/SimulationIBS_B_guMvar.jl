...
# for Plot 6a (resolution ratio over gT for different linear uM gradients guM>0)
params = Dict(
...
for (i, d) in enumerate(dicts)
    f = makesim(d)
    @tagsave(datadir("simulation", "SimulationIBS_B_guMvar", savename(d, "bson", scientific=3, ignores=(:char, :C, :retention_model, :retention_difference, :char_model, :comparison))), f, safe=false)
end
