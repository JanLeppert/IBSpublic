...
sys = IBS.System(10.0^(-0.4), -5, 1, 0, 1)
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_gT = IBS.ParTripletIBS(sys,sub,options)
solþξ_gT = Array{Any}(undef, 3)
sols²ξ_gT = Array{Any}(undef, 3)
solð²ξ_gT = Array{Any}(undef, 3)
for i=1:3
    solþξ_gT[i] = IBS.solving_migration(par_gT, n=i-2, ξ₀=ξ₀func)
    sols²ξ_gT[i] = IBS.solving_bandvariance(solþξ_IBS[i], par_gT, n=i-2, ξ₀=ξ₀func)
    solð²ξ_gT[i] = IBS.solving_peakvariance(solþξ_IBS[i], par_gT, n=i-2, ξ₀=ξ₀func)
end
...
sys = IBS.System(10.0^(-0.4), 0, 1, 1.9, 1)
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_guM = IBS.ParTripletIBS(sys,sub,options)
solþξ_guM = Array{Any}(undef, 3)
sols²ξ_guM = Array{Any}(undef, 3)
solð²ξ_guM = Array{Any}(undef, 3)
for i=1:3
    solþξ_guM[i] = IBS.solving_migration(par_guM, n=i-2, ξ₀=ξ₀func)
    sols²ξ_guM[i] = IBS.solving_bandvariance(solþξ_guM[i], par_guM, n=i-2, ξ₀=ξ₀func)
    solð²ξ_guM[i] = IBS.solving_peakvariance(solþξ_guM[i], par_guM, n=i-2, ξ₀=ξ₀func)
end
...
for i=1:length(þ₀_IBS)
    T₀_IBS[i] = IBS.temperature(ξ₀_IBS[i], þ₀_IBS[i], par_IBS)
end
T₀_guM = Array{Float64}(undef, length(þ₀_guM))
for i=1:length(þ₀_guM)
    T₀_guM[i] = IBS.temperature(ξ₀_guM[i], þ₀_guM[i], par_guM, ξ₀=ξ₀func(þ₀_guM[i]))
end
hold(false)
plot(þ₀_IBS, T₀_IBS)
hold(true)
...
sys = IBS.System(10.0^(-0.4), 0, 1, 0, 100)
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_P = IBS.ParTripletIBS(sys,sub,options)
solþξ_P = Array{Any}(undef, 3)
sols²ξ_P = Array{Any}(undef, 3)
solð²ξ_P = Array{Any}(undef, 3)
for i=1:3
    solþξ_P[i] = IBS.solving_migration(par_P, n=i-2, ξ₀=ξ₀func)
    sols²ξ_P[i] = IBS.solving_bandvariance(solþξ_P[i], par_P, n=i-2, ξ₀=ξ₀func)
    solð²ξ_P[i] = IBS.solving_peakvariance(solþξ_P[i], par_P, n=i-2, ξ₀=ξ₀func)
end
...
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_gT0 = IBS.ParTripletIBS(sys,sub,options)
solþξ_gT0 = Array{Any}(undef, 3)
sols²ξ_gT0 = Array{Any}(undef, 3)
solð²ξ_gT0 = Array{Any}(undef, 3)
for i=1:3
    solþξ_gT0[i] = IBS.solving_migration(par_gT0, n=i-2, ξ₀=zero)
    sols²ξ_gT0[i] = IBS.solving_bandvariance(solþξ_gT0[i], par_gT0, n=i-2, ξ₀=zero)
    solð²ξ_gT0[i] = IBS.solving_peakvariance(solþξ_gT0[i], par_gT0, n=i-2, ξ₀=zero)
end
ξ₀func_gT0 = IBS.trajectory(solþξ_gT0[2], ntraj=10000)
# solution for gT!=0
sys = IBS.System(10.0^(-0.4), -5, 1, 1.9, 1)
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_guMa = IBS.ParTripletIBS(sys,sub,options)
solþξ_guMa = Array{Any}(undef, 3)
sols²ξ_guMa = Array{Any}(undef, 3)
solð²ξ_guMa = Array{Any}(undef, 3)
for i=1:3
    solþξ_guMa[i] = IBS.solving_migration(par_guMa, n=i-2, ξ₀=ξ₀func_gT0)
    sols²ξ_guMa[i] = IBS.solving_bandvariance(solþξ_guMa[i], par_guMa, n=i-2, ξ₀=ξ₀func_gT0)
    solð²ξ_guMa[i] = IBS.solving_peakvariance(solþξ_guMa[i], par_guMa, n=i-2, ξ₀=ξ₀func_gT0)
end
# plot ξ₀func(þ₀) and ξ₀(þ₀) of IBS and non-IBS (guM=1.9)
...
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_gT0_19 = IBS.ParTripletIBS(sys,sub,options)
solþξ_gT0_19 = Array{Any}(undef, 3)
sols²ξ_gT0_19 = Array{Any}(undef, 3)
solð²ξ_gT0_19 = Array{Any}(undef, 3)
for i=1:3
    solþξ_gT0_19[i] = IBS.solving_migration(par_gT0_19, n=i-2, ξ₀=zero)
    sols²ξ_gT0_19[i] = IBS.solving_bandvariance(solþξ_gT0_19[i], par_gT0_19, n=i-2, ξ₀=zero)
    solð²ξ_gT0_19[i] = IBS.solving_peakvariance(solþξ_gT0_19[i], par_gT0_19, n=i-2, ξ₀=zero)
end
ξ₀func_gT0_19 = IBS.trajectory(solþξ_gT0_19[2], ntraj=10000)
# guM = 1.0
sys = IBS.System(10.0^(-0.4), 0, 1, 1.0, 1)
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_gT0_10 = IBS.ParTripletIBS(sys,sub,options)
solþξ_gT0_10 = Array{Any}(undef, 3)
sols²ξ_gT0_10 = Array{Any}(undef, 3)
solð²ξ_gT0_10 = Array{Any}(undef, 3)
for i=1:3
    solþξ_gT0_10[i] = IBS.solving_migration(par_gT0_10, n=i-2, ξ₀=zero)
    sols²ξ_gT0_10[i] = IBS.solving_bandvariance(solþξ_gT0_10[i], par_gT0_10, n=i-2, ξ₀=zero)
    solð²ξ_gT0_10[i] = IBS.solving_peakvariance(solþξ_gT0_10[i], par_gT0_10, n=i-2, ξ₀=zero)
end
ξ₀func_gT0_10 = IBS.trajectory(solþξ_gT0_10[2], ntraj=10000)
# guM = 0
sys = IBS.System(10.0^(-0.4), 0, 1, 0, 1)
sub = IBS.Substance(1e-5, 4, NaN, 0, 1/300, 0)
options = IBS.Options("ideal", "Δϑchar", "f(ϑchar)", "ξ₀")
par_gT0_0 = IBS.ParTripletIBS(sys,sub,options)
solþξ_gT0_0 = Array{Any}(undef, 3)
sols²ξ_gT0_0 = Array{Any}(undef, 3)
solð²ξ_gT0_0 = Array{Any}(undef, 3)
for i=1:3
    solþξ_gT0_0[i] = IBS.solving_migration(par_gT0_0, n=i-2, ξ₀=zero)
    sols²ξ_gT0_0[i] = IBS.solving_bandvariance(solþξ_gT0_0[i], par_gT0_0, n=i-2, ξ₀=zero)
    solð²ξ_gT0_0[i] = IBS.solving_peakvariance(solþξ_gT0_0[i], par_gT0_0, n=i-2, ξ₀=zero)
end
ξ₀func_gT0_0 = IBS.trajectory(solþξ_gT0_0[2], ntraj=10000)
# plot the ξ₀func
...
for i=1:length(þ₀_gT0_0)
    T₀_gT0_0[i] = IBS.temperature(ξ₀_gT0_0[i], þ₀_gT0_0[i], par_gT0_0, ξ₀=ξ₀func_gT0_0(þ₀_gT0_0[i]))
end
T₀_gT0_10 = Array{Float64}(undef, length(þ₀_gT0_10))
for i=1:length(þ₀_gT0_10)
    T₀_gT0_10[i] = IBS.temperature(ξ₀_gT0_10[i], þ₀_gT0_10[i], par_gT0_10, ξ₀=ξ₀func_gT0_10(þ₀_gT0_10[i]))
end
T₀_gT0_19 = Array{Float64}(undef, length(þ₀_gT0_19))
for i=1:length(þ₀_gT0_19)
    T₀_gT0_19[i] = IBS.temperature(ξ₀_gT0_19[i], þ₀_gT0_19[i], par_gT0_19, ξ₀=ξ₀func_gT0_19(þ₀_gT0_19[i]))
end
hold(false)
